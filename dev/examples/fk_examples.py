...
for i,muval in enumerate(murange):
	...
	L[1][i]= resnorm
	L[2][i]= solnorm
solverlist = ['cgs', 'cg', 'bicg']

st_rec_cg  = fk_reconstruct(sr90, slopes=[-3,3],maskshape=['butterworth', 4], slopepicking=False, method=None, solver='cg', mu=muval, tol=1e-15)
st_rec_cgs  = fk_reconstruct(sr90, slopes=[-3,3],maskshape=['butterworth', 4], slopepicking=False, method=None, solver='cgs', mu=muval, tol=1e-15)
st_rec_bicg  = fk_reconstruct(sr90, slopes=[-3,3],maskshape=['butterworth', 4], slopepicking=False, method=None, solver='bicg', mu=muval, tol=1e-15)
