...
from sipy.util.fkutil import  nextpow2, find_subsets, slope_distribution, makeMask, create_iFFT2mtx, plotfk
...
def test(x, case, nf=True):
	...
	bw_length = np.linspace(0, n_of_traces/2., n_of_traces/2.+1).astype('int')#[::2]
	...
	for length in bw_length:
		for slope in bw_slope:
			...
			for u in PC_u:
				...
				print('Q = %f, u = %f, k length = %f' % (Q, u/float(delta_traces), length),  end="\r")
				sys.stdout.flush()

	
	np.savetxt('../Qlist.dat', Qall)
