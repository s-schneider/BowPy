...
for i, noisefolder in enumerate(noisefoldlist):
	...
	for filein in rickerlist:
		...
		for alpha in alphalist:
			...
			plotname = PICPATH + 'pocs_' + str(noiselevellist[i]) + str(alpha) + '-alpha' + 'norm' + '.png'
			plotname2 = PICPATH + 'pocs_' + str(noiselevellist[i]) + str(alpha) + '-alpha' + 'diff' + '.png'
			...
			for maxiter in maxiterlist:
				...
				difflin.append(abs(data_org-drec).mean())		

				srs = array2stream(data).copy()
				st_rec = pocs_recon(srs, maxiter, method='denoise', decrease='exp', alpha=alpha)
				drecexp = stream2array(st_rec)
				recnormexp.append(np.linalg.norm(data_org-drecexp, 2))
				diffexp.append(abs(data_org-drec).mean())

			linplot.append([alpha, recnormlin, difflin])
			expplot.append([alpha, recnormexp, diffexp])

			plt.title("POCS with alpha = %f and maxiter = %i " % (np.round(alpha*1000.)/1000., maxiter) )
