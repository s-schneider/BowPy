...
maxiterlist = np.array([20]) #np.arange(50)
...
for i, noisefolder in enumerate(noisefoldlist):
	...
	for filein in rickerlist:
		...
		plotname = FPATH + FNAME + 'pocs_' + 'nlvl' + str(noiselevellist[i]) + '.png'
		plotnamelog = FPATH + FNAME + 'pocs_' + 'nlvl' + str(noiselevellist[i]) + '_logplot' '.png'
		...
		for alpha in alphalist:
			...
			for maxiter in maxiterlist:
				...
				Q = np.linalg.norm(data_org,2)**2. / np.linalg.norm(data_org - drecexp,2)**2.
				Qexp.append(Q)

		Qall.append([Qlin, Qexp])
		plt.close("all")
		plt.title("POCS with alpha = %s " % '{:01.2}'.format(alpha) )
		plt.xlabel('Alpha')
		plt.ylabel('Q')
