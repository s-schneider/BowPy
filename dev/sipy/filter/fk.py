...
def fk_reconstruct(st, slopes=[-3,3], deltaslope=0.05, slopepicking=False, smoothing=False, dist=0.5,maskshape=['boxcar',None], method='denoise', solver="iterative",  mu=5e-2, tol=1e-12, fulloutput=False):
	...
	if solver in ("lsqr", "leastsquares"):
		...
		Dv_rec = sparse.linalg.cgs(A, dv, tol=tol, maxiter=maxiter)
	elif solver in ('cg'):
		Dv_rec = sparse.linalg.cg(A, dv, x0=Dv, tol=tol, maxiter=maxiter)
	elif solver in ('bicg'):
		Dv_rec = sparse.linalg.bicg(A, dv, x0=Dv, tol=tol, maxiter=maxiter)
