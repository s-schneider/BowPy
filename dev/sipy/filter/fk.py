...
def pocs_recon(st, maxiter, method='denoise', decrease='linear', alpha=0.9):
	"""
	This functions reconstructs missing signals in the f-k domain, using the original data,
	...
	if method in ('recon'):
		...
		noft = ArrayData.shape[0]

	ADfinal = pocs(ArrayData, maxiter, noft, alpha, decrease)

	#datap = ADfinal.copy()

	st_rec 	= array2stream(ADfinal)
