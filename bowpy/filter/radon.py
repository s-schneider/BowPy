...
def radon_filter(
    st, inv, event, p, weights, line_model, inversion_model, hyperparameters
):
    """
    ...
    R, t, epi = radon_inverse(
        st_input, inv, event, p, weights, line_model, inversion_model, hyperparameters
    )
    ...
    Delta_resampled = np.arange(
        int(math.floor(min(epi))),
        int(math.ceil(max(epi))) + 1,
        (int(math.ceil(max(epi))) - int(math.floor(min(epi)))) / 20.0,
    )
    yticks = np.arange(
        int(math.ceil(min(Delta_resampled / 10))) * 10,
        int(math.ceil(max(Delta_resampled / 10))) * 10 + 10,
        10,
    )[::-1]
    xticks = np.arange(
        int(math.ceil(min(t / 100))) * 100,
        int(math.ceil(max(t / 100))) * 100 + 100,
        100,
    )[::2]
...
def radon_inverse(
    ...
    if (
        not isinstance(st, Stream)
        or not isinstance(inv, Inventory)
    ...
    t = np.linspace(
        0, st_tmp[0].stats.delta * st_tmp[0].stats.npts, st_tmp[0].stats.npts
    )
    it = t.size
    print(it)
    ...
    for k in range(ip):
        if line_model == "parabolic":
            Tshift[:, k] = (
                2.0 * ref_dist * Tshift[:, k] * Dist_array.conj().transpose()
    ...
    for i in range(int(math.floor((iF + 1) / 2))):
        ...
        if inversion_model in ("Cauchy", "L1"):
            ...
            if inversion_model == "Cauchy":
                COST_prev = np.linalg.norm(
                    Mfft[:, i] - dot(A, Rfft[:, i]), 2
                ) + lam * sum(np.log(abs(Rfft[:, i] ** 2 + b)))
            elif inversion_model == "L1":
                COST_prev = np.linalg.norm(
                    Mfft[:, i] - dot(A, Rfft[:, i]), 2
                ) + lam * np.linalg.norm(abs(Rfft[:, i] + 1), 1)
            itercount = 1
            ...
            while dCost > 0.001 and itercount < 10:
                ...
                if inversion_model == "Cauchy":
                    Q = sparse.spdiags(1.0 / (abs(Rfft[:, i] ** 2) + b), 0, ip, ip).A
                elif inversion_model == "L1":
                    Q = sparse.spdiags(1.0 / (abs(Rfft[:, i]) + b), 0, ip, ip).A
                Rfft[:, i] = sp.linalg.solve((lam * Q + AtA), AtM)

                # Determine change to cost function.
                if inversion_model == "Cauchy":
                    COST_cur = np.linalg.norm(
                        Mfft[:, i] - A * Rfft[:, i], 2
                    ) + lam * sum(np.log(abs(Rfft[:, i] ** 2 + b) - np.log(b)))
                elif inversion_model == "L1":
                    COST_cur = np.linalg.norm(
                        Mfft[:, i] - A * Rfft[:, i], 2
                    ) + lam * np.linalg.norm(abs(Rfft[:, i] + 1) + b, 1)
