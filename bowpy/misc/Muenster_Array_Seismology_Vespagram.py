...
def vespagram(
    ...
    great_cricle_dist, baz, az2 = gps2dist_azimuth(
        center_lat, center_lon, ev_lat, ev_lon
    )
    ...
    if plot_trace:
        ...
        for i in xrange(nbeam):
            if plot_max_beam_trace:
                if i == max_beam:
                    ax1.plot(
                        T, sll + scale * beams[i] / max_amp + i * sls, "r", zorder=1
                    )
                else:
                    ax1.plot(
                        T, sll + scale * beams[i] / max_amp + i * sls, "k", zorder=-1
                    )
            else:
        ...
        # plot the phase info
        ax1.scatter(
            phase_time_info, phase_slowness_info, s=2000, marker="|", lw=2, color="g"
        )
        for i, txt in enumerate(phase_name_info):
            ax1.annotate(
                txt,
                (phase_time_info[i], phase_slowness_info[i]),
                fontsize=18,
                color="r",
            )
    #####
...
def get_timeshift_baz(
    ...
    baz = math.pi * baze / 180.0  # BAZ converted from degrees to radiants
    nbeams = int((slm - sll) / sls + 0.5) + 1
    ...
    for k in xrange(nbeams):
        ...
        if static_correction:
            ...
            ) + sx * geometry[:, 2] / 1000 * 1.0 / np.tan(inc_ang_rad)
...
def plot_great_circle_path(
    ev_lon, ev_lat, ev_depth, center_lon, center_lat, baz, great_cricle_dist, model
):

    plt.figure(
        num=1, figsize=(17, 10), dpi=100
    ...
    bounce_dist = (
        bounce_vect[2] / 0.017455053237912375
    ...
    map = Basemap(
        projection="hammer",
        lon_0=bounce_lon_appx,
        lat_0=bounce_lat_appx,
        resolution="c",
    )
    map.drawcoastlines()
    # map.fillcontinents()
...
def align_phases(stream, event, inventory, phase_name, method="simple"):
    ...
    tt_1 = model.get_travel_times(
        distance_in_degree=tr_1.stats.distance,
    ...
    if plot_travel_times:
        ...
        for i in np.linspace(dist_min, dist_max, 1000):
            tts = model.get_travel_times(
                distance_in_degree=i, source_depth_in_km=event_depth_in_km
            )
            for phase in tts:
...
def plot_array_stations(stream, center_lon, center_lat, array_name):
    ...
    # draw parallels.
    parallels = np.arange(-90.0, 90.0, 1.0)
    map.drawparallels(parallels, labels=[1, 0, 0, 0], fontsize=10)
    # draw meridians
    meridians = np.arange(-180.0, 180.0, 2.0)
    ...
    for i, tr in enumerate(stream):
        ...
        # print(x,y)
        plt.annotate(
            tr.stats.coordinates.name, (x * 1.02, y * 1.02), fontsize=18, color="k"
        )
