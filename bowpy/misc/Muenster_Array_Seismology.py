...
from obspy.core.util.geodetics import (
    locations2degrees,
    gps2DistAzimuth,
    kilometer2degrees,
)
            ...
            for k, f in enumerate(np.arange(fmin, fmax + fstep / 10.0, fstep)):
                ...
                for l in np.arange(len(geometry)):
                    _sum += np.exp(
                        complex(
                            0.0,
                            (geometry[l, 0] * sx + geometry[l, 1] * sy) * 2 * np.pi * f,
                        )
                    )
                buff[k] = abs(_sum) ** 2
...
def array_analysis_helper(
    ...
    try:
        ...
        for i in xrange(numslice):
            slow_x = np.sin((baz[i] + 180.0) * np.pi / 180.0) * slow[i]
            slow_y = np.cos((baz[i] + 180.0) * np.pi / 180.0) * slow[i]
            ...
            if baz_plot:
                ...
                for ix, sx in enumerate(slx):
                    for iy, sy in enumerate(sly):
                        ...
                        if array_response:
                            ...
                            tbaz = (
                                np.arctan2(sx + slow_x, sy + slow_y) * 180 / np.pi
                                + 180.0
                            )
                            if tbaz > 180.0:
                                tbaz = -180.0 + (tbaz - 180.0)
                ...
                if array_response:
                    ...
                    transg = transgrid[:, 2]
                    trans = spi.griddata(
                        (tsl, tbz), transg, (grid_x, grid_y), method="nearest"
                    )
                ...
                if array_response:
                    ...
                    try:
                        ax.contour(tsly, tslx, transff.T, 5, colors="k", linewidth=0.5)
                    except:
                        pass
                ax.set_ylim(slx[0], slx[-1])
                ax.set_xlim(sly[0], sly[-1])
...
def get_spoint(stream, stime, etime):
    ...
    if (slatest - stime) > stream[0].stats.delta / 2.0:
        msg = "Specified start-time is smaller than starttime in stream"
        raise ValueError(msg)
    if (eearliest - etime) < -stream[0].stats.delta / 2.0:
        msg = "Specified end-time bigger is than endtime in stream"
        print(eearliest, etime)
        raise ValueError(msg)
    for i in xrange(nostat):
        offset = int(((stime - slatest) / stream[i].stats.delta + 1.0))
        negoffset = int(((eearliest - etime) / stream[i].stats.delta + 1.0))
...
def beamforming(
    ...
    geometry = get_geometry(
        stream, coordsys=coordsys, correct_3dplane=correct_3dplane, verbose=verbose
    )
    ...
    while eotr:
            ...
            for x in xrange(grdpts_x):
                for y in xrange(grdpts_y):
                    ...
                    for i in xrange(nstat):
                        ...
                        try:
                            shifted = sp.signal.hilbert(
                                stream[i].data[s + offset : s + nsamp + offset]
                            )
                            if len(shifted) < nsamp:
                                shifted = np.pad(
                                    shifted,
                    ...
                    for i in xrange(nstat):
                        ...
                        shifted = stream[i].data[s + offset : s + nsamp + offset]
                        singlet += 1.0 / nstat * np.sum(shifted * shifted)
                        beam += 1.0 / nstat * shifted * np.power(coh, nthroot)
                    bs = np.sum(beam * beam)
                    abspow_map[x, y] = bs / singlet
                    if abspow_map[x, y] > max_beam:
                        max_beam = abspow_map[x, y]
        ...
        if method == "SWP":
            ...
            beam_max = stream[0].data[spoint[0] + offset : spoint[0] + nsamp + offset]
...
def get_timeshift_baz(geometry, sll, slm, sls, baze, vel_cor=4.0, static_3D=False):
    """
    ...
    for k in xrange(nbeams):
        ...
        if vel_cor * sx < 1.0:
            ...
            inc = np.pi / 2.0
        time_shift_tbl[:, k] = sx * (
            geometry[:, 0] * math.sin(baz) + geometry[:, 1] * math.cos(baz)
        )
        if static_3D:
...
def vespagram_baz(
    ...
    time_shift_table = get_timeshift_baz(
        geometry, sll, slm, sls, baz, vel_cor=vel_cor, static_3D=static_3D
    )
    ...
    for x in xrange(nbeams):
        ...
        if method == "PWS":
            ...
            for i in xrange(nstat):
                ...
                shifted = stream[i].data[s : s + ndat]
                singlet += 1.0 / nstat * np.sum(shifted * shifted)
                beams[x] += 1.0 / nstat * shifted * np.power(coh, nthroot)
            bs = np.sum(beams[x] * beams[x])
            bs = bs / singlet
            if bs > max_beam:
                max_beam = bs
...
def show_distance_plot(
    stream, event, inventory, starttime, endtime, plot_travel_times=True
):
    """
    ...
    if plot_travel_times:
        ...
        for key in distances.iterkeys():
            ...
            if (
                min_tt_time >= times_array[-1]
                or max_tt_time <= times_array[0]
                or (max_distance - min_distance) < 0.8 * (dist_max - dist_min)
            ):
                continue
            ttime = ttimes[key]
            dist = distances[key]
...
def vespagram(
    ...
    if align:
        ...
        deg.append(kilometer2degrees(res[0] / 1000.0))
    ...
    #####
        ...
        ax1.imshow(
            beams, cmap=cmap, interpolation="nearest", extent=extent, aspect="auto"
        )

    ####
    result = "BAZ: %.2f Time %s" % (baz - 180.0, stt)
...
def get_timeshift(
    geometry, sll_x, sll_y, sl_s, grdpts_x, grdpts_y, vel_cor=4.0, static_3D=False
):
    """
        ...
        for k in xrange(grdpts_x):
            ...
            for l in xrange(grdpts_y):
                ...
                if vel_cor * slow <= 1.0:
                    ...
                    print(
                        "Warning correction velocity smaller than apparent " "velocity"
                    )
                    inc = np.pi / 2.0
                time_shift_tbl[:, k, l] = (
                    sx * geometry[:, 0]
...
def geometrical_center(inventory):
    ...
    return {
        "latitude": (np.max(lats) + np.min(lats)) / 2.0,
        "longitude": (np.max(lngs) + np.min(lngs)) / 2.0,
        "absolute_height_in_km": (np.max(hgt) + np.min(hgt)) / 2.0,
    }
    ...
    for i in range(len(lats)):
        for j in range(len(lats)):
            if lats[i] == lats[j]:
                continue
            distances.append(
                gps2DistAzimuth(lats[i], lngs[i], lats[j], lngs[j])[0] / 1000.0
            )
    return max(distances)


def find_closest_station(inventory, latitude, longitude, absolute_height_in_km=0.0):
    """
    ...
    for i in range(len(lats)):
        distance = np.sqrt(
            ((gps2DistAzimuth(lats[i], lngs[i], x, y)[0]) / 1000.0) ** 2
...
def plot_gcp(slat, slon, qlat, qlon, plat, plon, savefigure=None):
    ...
    try:
        for i in range(len(qlat)):
            m.drawgreatcircle(
                qlon[i], qlat[i], slon[i], slat[i], linewidth=1, color="black", zorder=1
            )
    except TypeError:
